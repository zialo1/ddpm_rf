...
NEW STRUCTURE (NOT WORKING)
Rectified Flow (64×64)
        ↓
provides structure prior
        ↓
Conditional DDPM SR model
        ↓
high-frequency refinement (128×128)

or



RF (64×64) ──► upsample ──► condition (128×128)
                                 │
noisy 128×128 x_t ───────────────┤
                                 ▼
                        Conditional U-Net
...
class RFDiscriminator(nn.Module):
    ...
    def forward(self, x):
        ...
        if x.dim() != 4:
            ...
            raise ValueError(
            f"Discriminator expects (B,3,H,W), got {x.shape}" )
...
class SRTrainer:
    ...
    def train_step(self, x64_real, x128, optimizer):
        ...
        # RF condition
        x64_rf = self.sampler.sample(B // 2)
        x64_rf = F.interpolate(x64_rf, size=128, mode="bilinear")
    
        x64_real = F.interpolate(x64_real, size=128, mode="bilinear")
    
        x64 = torch.cat([x64_real[:B//2], x64_rf], dim=0)
    
        perm = torch.randperm(B)
        x64 = x64[perm]
        x128 = x128[perm]
    
        t = torch.rand(B, device=x128.device)
        noise = torch.randn_like(x128)
    
        x_t = x128 * (1 - t[:, None, None, None]) + noise * t[:, None, None, None]
...
class HardRFReplayBuffer:
    ...
    # --------------------------------------------------------
    # REFRESH BUFFER (called occasionally, NOT every step)
    # --------------------------------------------------------
    @torch.no_grad()
    def refresh(self, batch_size=32):
        x = self.rf.sample(batch_size)
        scores = self.disc(x).squeeze()
    
        # convert to CPU once
        x = x.detach().cpu()
        scores = scores.detach().cpu()
    
        # keep temporary lists (NO overwrite of state)
        if not hasattr(self, "_img_list"):
            self._img_list = []
            self._score_list = []
    
        self._img_list.append(x)
        self._score_list.append(scores)
    
        # flatten
        self.images = torch.cat(self._img_list, dim=0)
        self.scores = torch.cat(self._score_list, dim=0)
    
        # trim buffer
        if self.images.size(0) > self.buffer_size:
            self.images = self.images[-self.buffer_size:]
            self.scores = self.scores[-self.buffer_size:]
    
            # also trim backing lists consistently
            self._img_list = [self.images]
            self._score_list = [self.scores]

        self._save_last(x, scores)

    # --------------------------------------------------------
    # FAST SAMPLING (NO RF CALLS HERE)
    # --------------------------------------------------------
    def sample(self, batch_size):
        if len(self.images) == 0:
            # fallback: generate initial buffer
            self.refresh(batch_size=batch_size)
    ...
    # OPTIONAL DEBUG SAVE
    # --------------------------------------------------------
    def _save_last(self, x, scores):
        x_vis = (x + 1) / 2

        fname = f"buf_{self.counter:06d}.png"
        vutils.save_image(x_vis, os.path.join(self.save_dir, fname), nrow=4)

        self.counter += 1
        
        
class Trainer:
    def __init__(self, model, rf_model, device, loader, save_dir="checkpoints"):
        self.device = device
        self.loader = loader

        # --- KEEP YOUR WORKFLOW EXACTLY ---
        self.rf = OneStepRF(rf_model, device)
        self.disc = RFDiscriminator().to(device)

        self.sampler = HardRFReplayBuffer(
        ...
        # optimizers (assumed external or simple default)
    ...
    def train(self, epochs=10):
        ...
        print("[Buffer] Warming up HardRFReplayBuffer...")
        
        for _ in range(5):
        
            self.sampler.refresh(batch_size=32)
        
        print("[Buffer] Ready.")
        for epoch in range(epochs):
            epoch_start = time.time()

            for i, (x64, x128) in enumerate(self.loader):
                ...
                # ------------------------------------------------
                if self.step % 2 == 0:
                    d_loss = self.disc_trainer.train_step(
                        x_real=x64,
                ...
                # 2. SR step
                # ------------------------------------------------
                sr_loss = self.sr_trainer.train_step(
                    x64_real=x64,
                    x128=x128,
                    optimizer=self.opt_sr
                )
                ...
                remaining = (epochs - epoch) * total_batches + (total_batches - i)

                eta = step_time * remaining

                print(
                    f"Epoch {epoch+1}/{epochs} | "
                    f"Step {self.step} | "
                    f"D loss {d_loss:.4f} | "
                    f"SR loss {sr_loss:.4f} | "
            ...
            print(f"[Epoch {epoch+1}] done in {self._fmt_time(epoch_time)}")

        # --------------------------------------------------------
        # SAVE MODEL
        # --------------------------------------------------------
        save_path = os.path.join(self.save_dir, "sr_final.pth")
        torch.save(self.model.state_dict(), save_path)

        print(f"\n[OK] Saved model → {save_path}")     
...
class SRUNet(nn.Module):
    def __init__(self, base=48):
        ...
        self.time_mlp = TimeEmbed(base)

        # input: x_t (3) + condition (3)
        self.in_conv = nn.Conv2d(6, base, 3, 1, 1)
        ...
        self.up2 = ResBlock(base * 4 + base * 2, base * 2)
        self.up1 = ResBlock(base * 2 + base, base)
    ...
    def forward(self, x_t, cond, t):
        ...
        u2 = self.up2(torch.cat([m, d1], dim=1))
        u1 = self.up1(torch.cat([u2, x], dim=1))
...
def main(amodel):
    ...
    # --------------------------------------------------------
    trainer = Trainer(
        model=SRUNet().to(device),  # your SR DDPM model
        rf_model=rf.model,
        device=device,
        loader=loader
    )
    ...
    print("SR MODEL TYPE:", type(trainer.model))
    # --------------------------------------------------------
    # 6. Train
    # --------------------------------------------------------
    trainer.train()
